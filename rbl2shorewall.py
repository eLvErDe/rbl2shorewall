...
def process(config):
    ...
    with open(config.shorewall6_blrules, "w") as shorewall6:
        shorewall6.write("#\n")
        shorewall6.write("#\n")
        shorewall6.write("# THIS FILE AS BEEN GENERATED BY %s %s AT %s" % (APP_NAME, APP_VER, datetime.datetime.now(tz=datetime.timezone.utc)))
        shorewall6.write("#\n")
        shorewall6.write("#\n")
